...
class DataLoad(object):
    """
    Performs DB Tables related actions: "create","delete","load","drop","exec" from SQLs in files
...
#---------------------------------------------------------------------------------------------
    def __init__(self,tables,connections=None,resources=None,RunTimeParameters=None,printrawlogs=True):
        """
        ...
        self.Mode=self.GetValue(tables,"Mode","Sequential")
        try:
        ...
        if resources:
            ...
            self.res_params={}
        self.ExecutionLog=[]
        self.printrawlogs=printrawlogs
#---------------------------------------------------------------------------------------------
...
#---------------------------------------------------------------------------------------------
    def getpar (self,d,prev_key="",fd={}):
        ...
        return fd
#---------------------------------------------------------------------------------------------
    def replace_Placeholders(self,sql,table_name,action=None,error=None):
        """
        ...
        try:
            ...
            if self.RunTimeParameters:
                for par in self.RunTimeParameters:
                    old = "#"+par+"#"
                    new = self.RunTimeParameters[par]
                    sql=sql.replace(old,new)
        except KeyError:
            pass
        return sql
#---------------------------------------------------------------------------------------------
    def ActionOnSuccess(self,TableName, Command,action=None,error=None):
        """
            Runs subprocess.call for a command (string) from json file
            and adds a log record
        """
        Command=self.replace_Placeholders(Command,TableName,action,error)
        self.ExecutionLogAdd (TableName,7,"Run the command onSuccess: ")
        self.ExecutionLogAdd (TableName,8,Command)
        result=subprocess.call(Command,shell=True)
        self.ExecutionLogAdd (TableName,9,"The command return code: %s" %result)
#---------------------------------------------------------------------------------------------
    def SendNotification(self, event, table="Notification",action=None,error=None):
        """
            Runs subprocess.call for a command (string) from json file
            and adds a log record
        """
        try:
            if event=="OnSuccess":
                Command=self.tables["OnSuccessNotification"]
            else:
                Command=self.tables["OnErrorNotification"]
            Command=self.replace_Placeholders(Command,table,action,error)
            self.ExecutionLogAdd (table,19,"Send notification on %s: "%event)
            self.ExecutionLogAdd (table,20,Command)
            result=subprocess.call(Command,shell=True)
            self.ExecutionLogAdd (table,21,"The command return code: %s" %result)
        except KeyError: #no notification is configured
            pass
#---------------------------------------------------------------------------------------------
    def ActionOnError(self,table,action,attempt,e):
        ...
        try:
            ...
            Command=GetAction(OnErrorActions)
        except KeyError:
            Command=None
        if not(Command): #Priority 2 - table
            try:
                OnErrorActions=table["OnError"]
                Command=GetAction(table["OnError"])
            except KeyError:
                Command=None
        if not(Command): #Priority 3 - all tables level
            try:
        ...
        if Command=="Stop":
            ...
            Command=self.replace_Placeholders(Command,table["name"],action,e)
            self.ExecutionLogAdd (table["name"],10,"Run the command: ")
            self.ExecutionLogAdd (table["name"],11,Command)
            result=subprocess.call([Command],shell=True)
            self.ExecutionLogAdd (table["name"],12,"The command return code: %s" %result)
            return 2
#---------------------------------------------------------------------------------------------
    def ExecuteSql(self,conn,cur,table,action,attempt=1,queue=None,ExecSQLFunc=None,ExecFetchSaveFunc=None):
        """
            ...
            In a case of an error the action can be repeated via onError command and send attempt (integer)
        ...
        if os.path.exists(script):
            sql=self.replace_Placeholders(open(script, "r").read(),table["name"],action)
            ...
            try:
                if ExecSQLFunc:
                    ...
                    cur.execute(sql)
                self.ExecutionLogAdd (table["name"],6,"SQL command is Done")
                if action=="extract":
                    if ExecFetchSaveFunc:
                        ExecFetchSaveFunc(sql,cur)
                    else:
                        rows = cur.fetchall()
                        #..........................................................................................
                        ToFile=self.GetValue(table["scripts"][action],"tofile","%s_%s.csv" %(table["name"],action))
                        delimeter=str(self.GetValue(table["scripts"][action],"delimeter",","))
                        quotechar=str(self.GetValue(table["scripts"][action],"quotechar","\""))
                        lineterminator=str(self.GetValue(table["scripts"][action],"lineterminator","\r\n"))
                        escapechar=str(self.GetValue(table["scripts"][action],"escapechar","\\"))
                        doublequote=self.GetValue(table["scripts"][action],"doublequote","True")
                        if doublequote=="True":
                            bdoublequote=True
                        else:
                            bdoublequote=False
                        quoting=self.GetValue(table["scripts"][action],"quoting","QUOTE_MINIMAL")
                        ...
                        #..........................................................................................
                        fp = open(ToFile, 'wb')
                        ExtractFile = csv.writer(fp
                        ,delimiter=delimeter
                        ,quotechar=quotechar
                        ,quoting=csv_quoting
                        ,doublequote=bdoublequote
                        ,lineterminator=lineterminator
                        ,escapechar=escapechar
                        )
                        ExtractFile.writerows(rows)                 
                conn.commit()
                #Trying to run onSuccess command
                try:
                    OnSuccessAction=table["scripts"][action]["OnSuccess"]
                    self.ActionOnSuccess(table["name"],OnSuccessAction,action)
                except KeyError: #no onSuccess command is configured
                    pass
                ...
                if error_f==1:
                    self.SendNotification("OnError",table["name"],action,error)
            finally:
                if cur:
                    cur.close()
        else:
            error="Cannot perform %s for %s using %s Script does not exist"%(action,table["name"],script)
            self.ExecutionLogAdd (table["name"],17,error)
            error_f=1
            self.SendNotification("OnError",table["name"],error)
        self.ExecutionLogAdd (table["name"],18,"====================================")
        if queue:
            queue.put(error_f)
        return error_f
#---------------------------------------------------------------------------------------------
    def ExecuteCLI(self,table,action,attempt=1,queue=None,ExecCLIFunc=None):
        """
            Runs performs action (exec), e.g. running an OS command using subprocess.call(script),
            table (a structure from JSON file with definition of the action to run, what to do on error or success).
            In a case of an error the action can be repeated via onError command and send attempt (integer)
            queue is used for a parallel execution to return the exit code back to the main thread
        """
        script=table["scripts"][action]["file"]
        self.ExecutionLogAdd (table["name"],1,"====================================")
        if os.path.exists(script):
            command=self.replace_Placeholders(open(script, "r").read(),table["name"],action)
            self.ExecutionLogAdd (table["name"],2,"Action: %s for %s from %s" %(action,table["name"],script))
            self.ExecutionLogAdd (table["name"],3,"................................")
            self.ExecutionLogAdd (table["name"],4,command)
            self.ExecutionLogAdd (table["name"],5,"................................")
            error_f=0
            try:
                if ExecCLIFunc:
                    error_f=ExecCLIFunc(command)
                else:
                    error_f=subprocess.check_call(command,shell=True)
                self.ExecutionLogAdd (table["name"],6,"CLI command is Done")
                #Trying to run onSuccess command
                try:
                    OnSuccessAction=table["scripts"][action]["OnSuccess"]
                    self.ActionOnSuccess(table["name"],OnSuccessAction,action)
        ...
        return error_f
#---------------------------------------------------------------------------------------------
    def run(self,action,StartingTable=None,attempt=1,ExecFunc=None,ExecFetchSaveFunc=None):
        """
        ...
        if self.Mode=="Parallel":
            ...
            while i<self.tables_num: #we need to execute action for each table
                Threads=[]
                queue = Queue.Queue()
                if action=="exec":#run an os command
                    table=self.tables["Tables"][i]
                    if not(self.GetValue(table["scripts"],action,None)):
                            #skip this table if there is no action defined
                            i+=1
                            continue
                    t = threading.Thread(target=self.ExecuteCLI, args=(table,action,attempt, queue,ExecFunc,))
                    Threads.append(t)
                    i+=1
                else: #run SQL command
                    j=0
                    ...
                    #but the number of provided connections can be less then the number of tables
                        ...
                        t = threading.Thread(target=self.ExecuteSql, args=(self.connections[j],cur,table,action,attempt, queue,ExecFunc,ExecFetchSaveFunc,))
                        Threads.append(t)
                        i+=1
                        j+=1
            ...
            for i in range(StartInd,self.tables_num):
                ...
                if not(self.GetValue(table["scripts"],action,None)):
                    ...
                    continue
                if action=="exec":#run an os command
                    result=self.ExecuteCLI(table,action,attempt,None,ExecFunc)
                    TotalResult=TotalResult+result
                else:#run sql command
                    cur = self.connections[0].cursor()
                    result=self.ExecuteSql(self.connections[0],cur,table,action,attempt,None,ExecFunc,ExecFetchSaveFunc)
                    TotalResult=TotalResult+result
                if result==1:
                    break
        if TotalResult==0:
            #Trying to run onSuccessaction specific command per action
            try:
                OnSuccessCommand=self.tables["OnSuccess"+action]
                self.ActionOnSuccess("OnSuccess"+action,OnSuccessCommand)
            except KeyError: #no onSuccessaction command is configured
                pass
            #Trying to run onSuccess command
            try:
                OnSuccessCommand=self.tables["OnSuccess"]
                self.ActionOnSuccess("OnSuccess",OnSuccessCommand)
            except KeyError: #no onSuccess command is configured
                pass
            #Sending notification:
            self.SendNotification("OnSuccess",action=action)
        return TotalResult
#---------------------------------------------------------------------------------------------
